...
function Reply(props) {
  ...
  const handleClick =async (e)=>{
    e.preventDefault();
    try {
        const res = await axios.post(`http://localhost:5000/api/contact/reply/${id}`,{message});
  ...
  const getContact =async ()=>{
   try {
      const {data} = await axios.post(`http://localhost:5000/api/contact/singlecontact/${id}`);
